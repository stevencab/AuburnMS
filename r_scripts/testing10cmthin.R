...
trees_thinned_10 <- list(plot1_thinned,
                         plot2_thinned,
                         plot3_thinned,
                         plot4_thinned,
                         plot5_thinned,
...
thinned_ba_10 <- trees_thinned_10 %>% 
  group_by(stem_id, plot, species, functional_group) %>% 
  summarise(dbh_cm = dbh,
            dbh_in = (dbh)*0.393701) %>% 
  summarise(ba_m2 = ((pi)*(dbh_cm/2)^2)/(10000),
...
before_x_after_ba <- merge(plot_basal_area, thinned_ba_10)

percent_ba_change <- before_x_after_ba %>% 
  group_by(plot) %>% 
  summarise(change_ba = ((1-(after_ba_ft2a/before_ba_ft2a))*100))

trt_change_10 <- merge(percent_ba_change, treatments_no_location)
treatments
...
meso_trt_change_10 <- merge(meso_percent_ba_change, treatments_no_location)




original_plots_10 <- ggplot(trees, aes(dbh, color = functional_group, fill = functional_group)) +
  geom_histogram() +
  scale_x_continuous(breaks = c(0,5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80)) +
  ggtitle("Trees DBH by Functional Group") +
  theme(plot.title = element_text(hjust = 0.5))

thinned_plots_10 <- ggplot(trees_thinned_10, aes(dbh, color = functional_group, fill = functional_group)) +
  geom_histogram() +
  scale_x_continuous(breaks = c(0,5,10,15,20,25,30,35,40,45,50,55,60,65,70,75,80)) +
...
change_total_ba_plots_10 <- ggplot(trt_change_10, aes(x = factor(thin_lvl, level = level_order), y = change_ba, color = thin_lvl)) +
  geom_boxplot() +
...
change_meso_ba_plots_10 <- ggplot(meso_trt_change_10, aes(x = factor(thin_lvl, level = level_order), y = change_ba, color = thin_lvl)) +
  geom_boxplot() +
  ggtitle("Change in Mesophyte BA 0-10 cm thin") +
  ylab("% Change BA") +
  xlab("Thin treatment level") +
...
test_10_12_15 <- plot_grid(change_meso_ba_plots_10, change_meso_ba_plots_12, change_meso_ba_plots_15, nrow = 1)
